class E502 {
    ...
    zamawiajacyZlecajacyPoleTekstowe(){
    ...
    wstepnaRezerwacjaPrzycisk(){
        return cy.get('button#resourceReservationBtn').contains('Wstępna rezerwacja')
    }

    zlozZamowieniePrzycisk(){
        return cy.get('button#sendToRealizationBtn').contains('Złóż zamówienie')
    }

    zaznaczWszystkieRezerwacjePrzyciskWyboru(){
        return cy.get('input.selectAllCheckboxes').first()
    }

    rezerwacjaPrzyciskWyboru(index){
        return cy.get('input.isSelectedCheckbox').eq(index)
    }
    ...
    sprawdzWidok(){
        ...
        this.zamawiajacyZlecajacyPoleTekstowe().should('be.visible')
    ...
    sprawdzWidokPodglad16(){
        cy.get('th[data-title="Anulowana?"]').first().click()
        this.daneRezerwacjiPierwszyPrzycisk().should('be.visible')
        this.szczegolyRezerwacjiPierwszyPrzycisk().should('be.visible')
        this.zasobyPierwszyPrzycisk().should('be.visible')
        this.historiaZmianPrzycisk().should('be.visible')
