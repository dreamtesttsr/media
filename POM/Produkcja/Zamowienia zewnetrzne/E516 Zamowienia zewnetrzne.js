...
class E516{
    ...
    {
        return cy.get('a[title="Przegląd wniosku o zasoby"]').contains('Z').first().scrollIntoView()
    }

    edycjaPierwszyPrzycisk()
    {
        return cy.get('a[title="Edycja"]').contains('E').first().scrollIntoView()
    }

    anulacjaZamowieniaPierwszyPrzycisk()
    {
        return cy.get('a[title="Anulacja zamówienia"]').contains('A').first().scrollIntoView()
    }

    powodAnulowaniaZamowieniaPopupPoleTekstowe(){
        return cy.get('textarea#CancellationReason')
    }

    potwierdzAnulowaniePopupPrzycisk(){
        return cy.get('button#CancelModal_modal-yesBtn').contains('Potwierdź')
    }    
    ...
    sprawdzWidok(){
        ...
        // this.przegladWnioskuoZasobyPierwszyPrzycisk().should('be.visible') // tylo dla rezerwacji
    }
    ...
    sprawdzFiltryZaawansowane(){
        ...
        this.zaawansowanePrzycisk().should('have.attr', 'aria-expanded', 'true').click()
        this.zaawansowanePrzycisk().should('have.attr', 'aria-expanded', 'false')
    }
