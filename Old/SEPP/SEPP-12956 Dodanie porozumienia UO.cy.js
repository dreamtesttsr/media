...
describe('SEPP-12956 Dodanie porozumienia UO', function () {
    const dzisiaj = DateTime.now().toFormat('dd.MM.yyyy')
    const rndStr = faker.lorem.words(3)
    it('Dodanie porozumienia UO', function () {
        ...
        cy.get('#TvAudition').type('test uo: ' + rndStr)
        ...
        e22.rodzajUslugiLista().select('UO - Usługa opracowania', {force: true})
        ...
        cy.get('[value="UO - Usługa opracowania"]').should('be.visible').and('have.prop', 'readOnly', true)
        cy.get('.fieldsetField').contains('Opracowanie / licencja')
