...
describe('SEPP-15272 Obsługa rezerwacji wstępnej', () => {
    ...
    it('Sprawdzenie widoczności zasobów rezerwacji wstępnej', () => {
        ...
        cy.log('Krok 9 - Przypisuje pracownika do jednego ze zleceń i weryfikuję możliwość jego modyfikacji')
        // Weryfikuję drugi dzień rezerwacji
        e503.nastepnyDzienPrzycisk().click()       
        fWspolne.sprawdzProgressBar()
        cy.get('.Gantt-event').contains('SEPP-15272')
        cy.get('#treeList > ul > li > ul > li').should('contain.text', 'RTF - TECHNIK STUDIA').and('contain.text', 'RTW - TECHNIK APARATURY').and('contain.text', 'Kran kamerowy').and('contain.text', 'Nagłośnienie').and('contain.text', 'Teleprompter') 

        e503.wybierzZasobPrzycisk('RTF - TECHNIK STUDIA').click()
        e503.dodajPracownikaPrzycisk().click()
        fWspolne.sprawdzProgressBar()
        cy.get('[title="test_user_15 Imię_15 (111111111111)"').siblings('input[type="checkbox"]').check()
        e50301.zapiszPrzycisk().click()
        fWspolne.sprawdzProgressBar()
        e50301.powrotPrzycisk().click()
        e503.wybierzZasobPrzycisk('test_user_15, Imię_15 (111111111111)').click()
        e503.modyfikujPrzypisanieZasobuPrzycisk().should('be.visible').click()
        e50301.powrotPrzycisk().should('be.visible').click()            
