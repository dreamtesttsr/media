...
describe('SEPP-14608 Rola 29', () => {
    ...
    it('Rola 29 - Administrator biznesowy CUP (cennik)', () => {
        ...
        e50601.sprawdzURL()
        cy.get('#trackDiv > h3').should('contain.text', '0')
        e50601.sprawdzWidok()
        e50601.sprawdzWidok29()
        e50601.powrotPrzycisk().click()
        let positionID
        cy.get('#priceListItems_table > tbody > tr:nth-child(1) > td:nth-child(1)')
            .invoke('text')
            .then((id) => {
                positionID = id
            })
        e506.podgladPierwszyPrzycisk().click()
        cy.get('#trackDiv > h3').should(($id) => {
            expect($id).to.contain.text(positionID)
        })
